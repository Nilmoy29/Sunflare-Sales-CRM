...
import {
  followUpQueueReasonLabel,
  getFollowUpQueueReason,
...
function queueReasonBadgeClass(
  reason: NonNullable<ReturnType<typeof getFollowUpQueueReason>>,
): string {
  switch (reason) {
    case "overdue":
      return "bg-red-500/15 text-red-400";
...
export function PipelineFollowUpCell({
  ...
  const queueReason = getFollowUpQueueReason(lead);
  ...
  return (
    <div className="min-w-[14rem] space-y-2">
      {queueReason ? (
        <span
          className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${queueReasonBadgeClass(queueReason)}`}
        >
          {followUpQueueReasonLabel(queueReason)}
        </span>
      ) : null}
      ...
      {hasScheduledFollowUp ? (
        <div className="space-y-1 rounded-md border border-border bg-secondary/30 p-2">
          ...
          {followUpNote ? (
            <p className="line-clamp-3 text-xs text-foreground" title={followUpNote}>
              {followUpNote}
            </p>
          ) : null}
          <button
        ...
        <p className="text-xs text-muted-foreground">
          Mark done after closing out, or update status from the row.
        </p>
      ) : null}
      ...
      <div className="space-y-1.5">
        <input
          ...
          disabled={disabled || saving || completing}
          className="min-h-9 w-full rounded border border-border bg-background px-2 py-1 text-xs text-foreground [color-scheme:dark]"
        />
        <textarea
